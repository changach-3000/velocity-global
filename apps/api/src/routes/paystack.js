...
const authenticatePocketBase = async () => {
  try {
    if (!pb.authStore.isValid) {
      console.log(`[${getTimestamp()}] [Paystack] Authenticating with PocketBase...`);
      await pb.collection('users').authWithPassword(
        process.env.POCKETBASE_ADMIN_EMAIL,
        process.env.POCKETBASE_ADMIN_PASSWORD
      );
      console.log(`[${getTimestamp()}] [Paystack] PocketBase authentication successful`);
    }
  } catch (error) {
    console.error(`[${getTimestamp()}] [Paystack] PocketBase authentication failed:`, error.message);
    throw new Error('Database authentication failed: ' + error.message);
...
router.post('/initialize', async (req, res) => {
  ...
  try {
    const { courseId, cartItems, userId, userEmail, amount, promoCode } = req.body;
    ...
    if (finalAmount === 0) {
      console.log(`[${getTimestamp()}] [Paystack] Free checkout — enrolling directly without payment`);

      await authenticatePocketBase();

      const promoReference = `PROMO-${(promoCode || 'FREE').toUpperCase()}-${Date.now()}`;
      const { enrollments, enrollmentErrors } = await createEnrollments(coursesToPurchase, userId, promoReference);

      if (enrollments.length > 0) {
        console.log(`[${getTimestamp()}] [Paystack] Free enrollment successful for ${enrollments.length} course(s)`);
        return res.status(200).json({
          free: true,
          enrollments,
          errors: enrollmentErrors.length > 0 ? enrollmentErrors : undefined,
    ...
    console.log(
      `[${getTimestamp()}] [Paystack] USD: ${finalAmount} → KES: ${amountInKes} (rate: ${USD_TO_KES_RATE}) → kobo: ${amountInKobo}`
    );
...
router.post('/verify', async (req, res) => {
  ...
  try {
    ...
    const { enrollments, enrollmentErrors } = await createEnrollments(coursesToEnroll, userId, reference);
